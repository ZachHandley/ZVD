...
impl RtmpHandshake {
    ...
    pub fn perform_simple_handshake<S: Read + Write>(stream: &mut S) -> Result<()> {
        ...
        stream.write_all(&c1).map_err(|e| Error::Io(e))?;
        stream.flush().map_err(|e| Error::Io(e))?;

        // S0: Read server version
        let mut s0 = [0u8; 1];
        stream.read_exact(&mut s0).map_err(|e| Error::Io(e))?;
        ...
        stream.read_exact(&mut s1).map_err(|e| Error::Io(e))?;

        // C2: Echo S1 back to server
        stream.write_all(&s1).map_err(|e| Error::Io(e))?;
        stream.flush().map_err(|e| Error::Io(e))?;

        // S2: Read server's echo (1536 bytes)
        let mut s2 = vec![0u8; 1536];
        stream.read_exact(&mut s2).map_err(|e| Error::Io(e))?;
    ...
    pub fn perform_simple_handshake_split<W: Write, R: Read>(
        writer: &mut W,
        reader: &mut R,
    ) -> Result<()> {
        // C0: Send version byte
        ...
        writer.write_all(&c1).map_err(|e| Error::Io(e))?;
        writer.flush().map_err(|e| Error::Io(e))?;

        // S0: Read server version
        let mut s0 = [0u8; 1];
        reader.read_exact(&mut s0).map_err(|e| Error::Io(e))?;
        ...
        // S1: Read server handshake (1536 bytes)
        let mut s1 = vec![0u8; 1536];
        reader.read_exact(&mut s1).map_err(|e| Error::Io(e))?;

        // C2: Echo S1 back to server
        writer.write_all(&s1).map_err(|e| Error::Io(e))?;
        writer.flush().map_err(|e| Error::Io(e))?;

        // S2: Read server's echo (1536 bytes)
        let mut s2 = vec![0u8; 1536];
        reader.read_exact(&mut s2).map_err(|e| Error::Io(e))?;
...
impl ChunkStream {
    ...
    pub fn write_message<W: Write>(
        ...
        while offset < payload.len() {
            ...
            writer.write_all(&header_bytes).map_err(|e| Error::Io(e))?;
