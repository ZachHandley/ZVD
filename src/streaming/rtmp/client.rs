...
impl RtmpClient {
    ...
    pub fn connect(&mut self, rtmp_url: &str, stream_key: &str) -> Result<()> {
        ...
        // Connect TCP
        let tcp = TcpStream::connect(format!("{}:{}", host, port)).map_err(|e| Error::Io(e))?;
        ...
        self.chunk_stream
            .write_message(&mut writer, 3, &connect_msg)?;
        ...
        self.chunk_stream
            .write_message(&mut writer, 3, &release_msg)?;
        writer.flush().map_err(|e| Error::Io(e))?;
        ...
        self.chunk_stream
            .write_message(&mut writer, 3, &fc_publish_msg)?;
        writer.flush().map_err(|e| Error::Io(e))?;
        ...
        self.chunk_stream
            .write_message(&mut writer, 3, &create_stream_msg)?;
        writer.flush().map_err(|e| Error::Io(e))?;
        ...
        self.chunk_stream
            .write_message(&mut writer, 3, &publish_msg)?;
        writer.flush().map_err(|e| Error::Io(e))?;
    ...
    pub fn send_video_packet(
        ...
        if !self.connected {
            return Err(Error::InvalidState(
                "Not connected to RTMP server".to_string(),
            ));
        }
...
impl crate::streaming::StreamSession for RtmpClient {
    ...
    fn send_packet(&mut self, packet: &crate::format::Packet) -> Result<()> {
        ...
        match packet.codec_type {
            CodecType::Video => {
                ...
                let is_keyframe = packet.flags.keyframe;
                let timestamp = if packet.pts.is_valid() {
                    packet.pts.value as u32
                } else {
                    0
                };
                self.send_video_packet(packet.data.as_slice(), timestamp, is_keyframe)
            }
            CodecType::Audio => {
                let timestamp = if packet.pts.is_valid() {
                    packet.pts.value as u32
                ...
                self.send_audio_packet(packet.data.as_slice(), timestamp)
            }
    ...
    fn receive_packet(&mut self) -> Result<crate::format::Packet> {
        ...
        Err(Error::Unsupported(
            "RTMP client does not support receiving packets".to_string(),
        ))
    }
...
mod tests {
    ...
    fn test_rtmp_url_parsing() {
        ...
        let (host, port, app) =
            RtmpClient::parse_rtmp_url("rtmp://localhost:1936/app/stream").unwrap();
