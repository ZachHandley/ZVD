...
use crate::format::{Muxer, Packet, Stream, StreamInfo};
use std::fs::File;
...
impl Segmenter {
    ...
    pub fn new(config: SegmentConfig) -> Result<Self> {
        ...
        std::fs::create_dir_all(&config.output_dir).map_err(|e| Error::Io(e))?;
    ...
    fn close_segment(&mut self) -> Result<()> {
        if let Some(mut segment) = self.current_segment.take() {
            ...
            // Get file size
            let metadata = std::fs::metadata(&segment.path).map_err(|e| Error::Io(e))?;
    ...
    pub fn write_packet(&mut self, packet: &Packet) -> Result<()> {
        ...
        let should_split = (packet.flags.keyframe
    ...
    fn test_segment_splitting() {
        ...
        for i in 0..20 {
            ...
            let packet = Packet {
                ...
                data: Buffer::from_vec(vec![0; 100]),
                pts: Timestamp::new(pts_ms),
                dts: Timestamp::new(pts_ms),
                duration: 1000,
                flags: PacketFlags {
