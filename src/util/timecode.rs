...
impl Timecode {
    ...
    pub fn new(hours: u8, minutes: u8, seconds: u8, frames: u8, frame_rate: FrameRate) -> Result<Self> {
        // Validate ranges
        if hours > 23 {
            return Err(Error::InvalidInput(format!("Hours must be 0-23, got {}", hours)));
        }
        if minutes > 59 {
            return Err(Error::InvalidInput(format!("Minutes must be 0-59, got {}", minutes)));
        }
        if seconds > 59 {
            return Err(Error::InvalidInput(format!("Seconds must be 0-59, got {}", seconds)));
        }
...
impl fmt::Display for Timecode {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        write!(
            f,
            "{:02}:{:02}:{:02}{}{}",
            self.hours, self.minutes, self.seconds, separator, format!("{:02}", self.frames)
        )
    }
}
