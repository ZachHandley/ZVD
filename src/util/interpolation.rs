...
impl FrameData {
    ...
    pub fn y_plane(&self) -> &[u8] {
        match self.format {
            FrameFormat::YUV420P | FrameFormat::YUV444P => &self.data[0..self.width * self.height],
            _ => &[],
        }
    }
}
...
impl FrameInterpolator {
    ...
    pub fn interpolate(&self, frame1: &FrameData, frame2: &FrameData, t: f64) -> Result<FrameData> {
    ...
    fn find_best_match(
        ...
        for dy in -self.search_range..=self.search_range {
            for dx in -self.search_range..=self.search_range {
                let cost =
                    self.calculate_sad(frame1, frame2, width, height, block_x, block_y, dx, dy);
