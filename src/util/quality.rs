...
fn calculate_ssim_plane(
    ...
    for y in (0..height - BLOCK_SIZE + 1).step_by(STRIDE) {
        for x in (0..width - BLOCK_SIZE + 1).step_by(STRIDE) {
            let ssim_block = calculate_ssim_block(
                reference,
                distorted,
                width,
                x,
...
fn calculate_ssim_block(
    ...
    let denominator =
        (mu_ref * mu_ref + mu_dist * mu_dist + c1) * (var_ref + var_dist + c2);
...
mod tests {
    ...
    fn test_mse_to_psnr() {
        ...
        // PSNR = 10 * log10(255^2 / 25) ≈ 38.13 dB
        assert!((psnr - 38.13).abs() < 0.01);
