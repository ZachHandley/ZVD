...
impl SyncDetector {
    ...
    fn cross_correlation(&self, audio1: &[f32], audio2: &[f32]) -> Result<SyncResult> {
        ...
        for offset in 0..max_search {
            let correlation = self
                .calculate_correlation(&reference[0..ref_len], &search[offset..offset + ref_len]);
    ...
    pub fn detect_drift(&self, audio1: &[f32], audio2: &[f32], sample_rate: u32) -> Result<f64> {
...
mod tests {
    ...
    fn test_cross_correlation_identical() {
        // Use different lengths so the correlation algorithm can search
        let audio1 = create_test_audio(1000, 440.0, 48000);
        let mut audio2 = audio1.clone();
        audio2.extend(vec![0.0; 100]); // Make audio2 slightly longer

        let detector = SyncDetector::new(SyncMethod::CrossCorrelation).with_max_offset(1000);

        let result = detector.detect_offset(&audio1, &audio2).unwrap();

        // Offset should be 0 or very small since audio starts the same
        assert!(result.offset_samples.abs() < 50);
    ...
    fn test_spike_detection() {
        // Create audio long enough to satisfy min_spacing (4800 samples @ 48kHz = 100ms)
        let mut audio = vec![0.1f32; 10000];
        audio[5000] = 1.0; // Spike at position 5000 (past min_spacing)

        let detector = SyncDetector::new(SyncMethod::SpikeDetection);
        let spikes = detector.detect_spikes(&audio);

        assert!(!spikes.is_empty());
        assert_eq!(spikes[0].position, 5000);
    }

    #[test]
    fn test_spike_sync_detection() {
        // Create audio long enough to satisfy min_spacing (4800 samples @ 48kHz = 100ms)
        let mut audio1 = vec![0.1f32; 12000];
        let mut audio2 = vec![0.1f32; 12000];

        audio1[5000] = 1.0; // Clap at 5000
        audio2[5200] = 1.0; // Clap at 5200 (200 samples later)
