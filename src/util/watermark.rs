    ...
    pub fn calculate_position(
        ...
        match self {
            ...
            WatermarkPosition::BottomLeft => {
                (margin, frame_height as i32 - watermark_height as i32 - margin)
            }
...
impl Watermark {
    ...
    pub fn apply(&self, frame_rgb: &mut [u8], width: usize, height: usize) -> Result<()> {
        match &self.watermark_type {
            ...
            WatermarkType::Image { data, width: wm_width, height: wm_height } => {
    ...
    fn apply_text_watermark(
        ...
        let (x, y) = self.position.calculate_position(width, height, text_width, text_height);
    ...
    /// Draw rectangle
    fn draw_rect(&self, frame: &mut [u8], width: usize, x: i32, y: i32, w: i32, h: i32, color: [u8; 3]) {
        for dy in 0..h {
            for dx in 0..w {
                let px = x + dx;
                let py = y + dy;
                        ...
                        match self.style {
                            ...
                            WatermarkStyle::SemiTransparent { opacity } => {
                                frame[idx] = ((1.0 - opacity) * frame[idx] as f32
                                    + opacity * color[0] as f32) as u8;
                                frame[idx + 1] = ((1.0 - opacity) * frame[idx + 1] as f32
                                    + opacity * color[1] as f32) as u8;
                                frame[idx + 2] = ((1.0 - opacity) * frame[idx + 2] as f32
                                    + opacity * color[2] as f32) as u8;
                            }
                            WatermarkStyle::Additive { strength } => {
                                frame[idx] = (frame[idx] as f32 + strength * color[0] as f32)
                                    .min(255.0) as u8;
                                frame[idx + 1] = (frame[idx + 1] as f32 + strength * color[1] as f32)
                                    .min(255.0) as u8;
                                frame[idx + 2] = (frame[idx + 2] as f32 + strength * color[2] as f32)
                                    .min(255.0) as u8;
                            }
                        }
                    }
    ...
    fn apply_image_watermark(
        ...
        let (x, y) = self.position.calculate_position(width, height, wm_width, wm_height);
            ...
            for dx in 0..wm_width {
                ...
                if px >= 0 && px < frame_width as i32 && py >= 0 {
                    ...
                    if frame_idx + 2 < frame.len() && wm_idx + 2 < watermark.len() {
                        let wm_color = [watermark[wm_idx], watermark[wm_idx + 1], watermark[wm_idx + 2]];
    ...
    fn blend_pixel(&self, frame: &mut [u8], idx: usize, color: [u8; 3]) {
        match self.style {
            ...
            WatermarkStyle::SemiTransparent { opacity } => {
                frame[idx] = ((1.0 - opacity) * frame[idx] as f32 + opacity * color[0] as f32) as u8;
                frame[idx + 1] = ((1.0 - opacity) * frame[idx + 1] as f32 + opacity * color[1] as f32) as u8;
                frame[idx + 2] = ((1.0 - opacity) * frame[idx + 2] as f32 + opacity * color[2] as f32) as u8;
            }
            WatermarkStyle::Additive { strength } => {
                frame[idx] = (frame[idx] as f32 + strength * color[0] as f32).min(255.0) as u8;
                frame[idx + 1] = (frame[idx + 1] as f32 + strength * color[1] as f32).min(255.0) as u8;
                frame[idx + 2] = (frame[idx + 2] as f32 + strength * color[2] as f32).min(255.0) as u8;
            }
        }
    }
...
mod tests {
    ...
    fn test_visible_text_watermark() {
        let watermark = Watermark::new_visible_text(
            "Test",
