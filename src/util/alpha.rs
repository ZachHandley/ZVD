...
impl AlphaCompositor {
    ...
    fn composite_add(&self, fg: &RgbaFrame, bg: &RgbaFrame, output: &mut RgbaFrame) {
        for i in 0..fg.data.len() / 4 {
            ...
            for c in 0..3 {
                output.data[idx + c] =
                    (fg.data[idx + c] as u16 + bg.data[idx + c] as u16).min(255) as u8;
            }

            output.data[idx + 3] =
                ((fg.data[idx + 3] as u16 + bg.data[idx + 3] as u16).min(255)) as u8;
        }
    }
...
mod tests {
    ...
    fn test_composite_add() {
        let mut fg = RgbaFrame::new(1, 1, AlphaType::Straight);
        fg.data[0] = 100; // R
        fg.data[3] = 255; // Full alpha (required for premultiplied conversion)

        let mut bg = RgbaFrame::new(1, 1, AlphaType::Straight);
        bg.data[0] = 100; // R
        bg.data[3] = 255; // Full alpha
