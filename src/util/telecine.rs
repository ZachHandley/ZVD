...
impl InterlacedFrame {
    ...
    pub fn to_progressive(&self) -> Vec<u8> {
        ...
        for y in 0..self.height {
            ...
            if y % 2 == 0 {
                // Top field
                progressive[row_start..row_end]
                    .copy_from_slice(&self.top_field[field_row..field_row + self.width * 3]);
            } else {
                // Bottom field
                progressive[row_start..row_end]
                    .copy_from_slice(&self.bottom_field[field_row..field_row + self.width * 3]);
            }
        }
...
mod tests {
    ...
    fn test_telecine_with_field_order() {
        let telecine =
            Telecine::new(PulldownPattern::ThreeTwo).with_field_order(FieldOrder::BottomFieldFirst);
    ...
    fn test_inverse_telecine() {
        ...
        assert_eq!(pulled.len(), 10); // 3+2+3+2 = 10 frames

        let restored = telecine.inverse_telecine(&pulled, 64, 64).unwrap();

        // Should restore to approximately 4 frames (may have some variation due to cadence detection)
        // The key is that the frame count is reduced from 10 to around 4-5
        assert!(restored.len() >= 4 && restored.len() <= 5);
    }
