...
mod tests {
    ...
    fn test_vu_meter_db() {
        let mut vu = VuMeter::new(48000);
        // VU meter has 300ms ballistics, so we need enough samples
        // At 48kHz, 300ms = 14400 samples. Use more for reliable settling.
        let signal = vec![0.5f32; 48000]; // 1 second of audio
        vu.process(&signal);

        let db = vu.get_db();
        assert!(db < 0.0); // 0.5 linear is -6 dB
        assert!(db > -12.0); // Ballistic meter will settle close to -6 dB
    }
