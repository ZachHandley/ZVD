...
impl LoudnessMeter {
    ...
    pub fn process_samples(&mut self, samples: &[f32], channels: usize) -> Result<()> {
        ...
        for frame in samples.chunks_exact(channels) {
            ...
            let weighted_power = if channels >= 5 {
                ...
                channel_powers[0] + channel_powers[1] + channel_powers[2]
    ...
    fn process_block(&mut self, channels: usize) {
        ...
        let mean_square: f64 = self.current_block.iter().sum::<f64>() / self.current_block.len() as f64;
    ...
    pub fn integrated_loudness(&self) -> f64 {
        ...
        let mean_loudness: f64 = gated_blocks.iter().map(|b| b.loudness).sum::<f64>()
...
mod tests {
    ...
    fn test_true_peak_limiter() {
        let mut normalizer = LoudnessNormalizer::new(48000, -10.0);
        normalizer.set_max_true_peak(-1.0);

        // Generate loud tone at -12 LUFS
        ...
        // Gain should be limited to avoid exceeding -1.0 dBTP
        assert!(gain <= 1.0); // Should not boost (would exceed true peak limit)
    }
