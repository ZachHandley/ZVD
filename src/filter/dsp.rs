    ...
    fn calculate_gain_reduction(&self, level_db: f64) -> f64 {
        if self.knee_db > 0.0 {
            ...
            if level_db < (self.threshold_db - self.knee_db / 2.0) {
                ...
                let gain_reduction =
                    diff * diff / (2.0 * self.knee_db) * (1.0 - 1.0 / self.ratio);
                -gain_reduction
            }
        } else {
            // Hard knee
...
mod tests {
    ...
    fn test_expander() {
        ...
        exp.set_threshold(-40.0);
        exp.set_ratio(2.0);

        // Quiet signal should be attenuated more
        let quiet_input = 0.01f32;
        let output = exp.process(quiet_input);
