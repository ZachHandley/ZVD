...
mod tests {
    ...
    fn test_linear_resampling_upsample() {
        ...
        // Use longer input for more reliable upsampling result
        let input: Vec<f32> = (0..100).map(|i| (i as f32 / 10.0).sin()).collect();
        let output = resampler.process(&input).unwrap();

        // Output should be longer (upsampling ratio is ~1.088)
        // With buffer handling, should produce approximately input.len() * ratio samples
        assert!(output.len() >= (input.len() as f64 * 0.9) as usize);
    }
    ...
    fn test_sinc_resampling() {
        ...
        // Use longer input to account for window size overhead
        let input: Vec<f32> = (0..500).map(|i| (i as f32 / 10.0).sin()).collect();
        let output = resampler.process(&input).unwrap();

        // Sinc resampler uses a window, so output may be smaller due to edge handling
        // But should still produce significant output
        assert!(!output.is_empty());
        assert!(output.len() > input.len() / 2); // Should produce substantial output
    }
