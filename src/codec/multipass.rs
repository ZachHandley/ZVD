...
impl PassStatistics {
    ...
    pub fn new(target_bitrate: u32, framerate: f64, gop_size: u32, rate_control: RateControlMode) -> Self {
        PassStatistics {
            frame_count: 0,
    ...
    pub fn calculate_frame_bitrate(&self, frame_index: usize, lookahead_window: usize) -> u32 {
        ...
        let weighted_complexity = (frame.complexity * 0.7 + lookahead_complexity * 0.3) / self.avg_complexity;
...
impl MultiPassEncoder {
    ...
    pub fn record_frame(&mut self, frame_stats: FrameStats) -> Result<()> {
        ...
        if let Some(ref mut stats) = self.statistics {
            ...
            Err(Error::InvalidInput("Statistics not initialized".to_string()))
        }
    }
    ...
    pub fn finalize(&mut self) -> Result<()> {
        if self.pass_type.is_first_pass() {
            if let Some(ref mut stats) = self.statistics {
                ...
                println!("First pass complete. Statistics saved to: {:?}", self.stats_file);
